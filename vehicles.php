...
if (isset($_GET['buy-id'])) {
    ...
    $queryvs = mysqli_query($connect, "SELECT * FROM `vehicles` INNER JOIN `vehicle_categories` ON vehicles.category_id=vehicle_categories.id WHERE vehicles.id = '$vehicle_id' LIMIT 1");
    ...
    if ($countvs > 0) {
        ...
        if ($rowvs['type'] == 'Garage') {
            ...
            while ($rowpv = mysqli_fetch_assoc($querypv)) {
                ...
                $queryvcc = mysqli_query($connect, "SELECT category_id, type FROM `vehicles` INNER JOIN `vehicle_categories` ON vehicles.category_id=vehicle_categories.id WHERE vehicles.id='$vh_id' LIMIT 1");
                $rowvcc   = mysqli_fetch_assoc($queryvcc);
                if ($rowvcc['type'] == 'Garage') {
            ...
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Vehicle Showroom</h5>
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    </div>
                    <div class="modal-body">
                        <center>
                            <h5><span class="badge badge-danger">' . lang_key("vno-freeslots") . '</span></h5>
        ...
        if ($rowu['money'] >= $rowvs['money'] && $rowu['gold'] >= $rowvs['gold'] && $rowu['level'] >= $rowvs['min_level'] && $free_slots > 0 && $owned == 'No' && $vippr == 'No') {
...
VALUES ('$player_id', '$vehicle_id', '$speed', '$acceleration', '$stability')");
        ...
        <div id="buy-vehicle" class="modal fade">
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">' . lang_key("vehicle-showroom") . '</h5>
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    </div>
                    <div class="modal-body">
                        <center>
                            <h5><span class="badge badge-info">' . lang_key("success-vpurchase") . '</span></h5>
                            <img src="' . $rowvs['image'] . '" width="80%"><br />
                            <br /><br />
                            <div class="row">
                                <div class="col-md-4">
...
<div class="card">
    <div class="card-header bg-default">
        <div class="row">
            <div class="col-md-2">
                <h5 class="card-title"><i class="fa fa-car"></i> <?php
...
?>">
                <div class="card-body" style="background-image: url(images/backgrounds/showroom/1.jpg); background-size: 100% 100%; background-repeat: no-repeat; height: auto;">
                    <center>
                        <img src="<?php
            echo $rowv['image'];
?>" <?php
...
<?php
            $vehicle_id = $rowv['id'];
            $player_id  = $rowu['id'];
            $querypvc   = mysqli_query($connect, "SELECT * FROM `player_vehicles` WHERE vehicle_id='$vehicle_id' AND player_id='$player_id' LIMIT 1");
...
?>
                        ...
                        <div class="col-md-4">
                            <div class="float-right"><i class="far fa-money-bill-alt"></i> <?php
            echo lang_key("money");
...
?></span><?php
            if ($rowv['vip'] == 'Yes') {
                echo '&nbsp;&nbsp;&nbsp; <span class="badge badge-primary"><i class="fa fa-star"></i> ' . lang_key("vip-only") . '</span>';
            }
?></div>
                        </div>
...
?>
            ...
            <a class="left carousel-control-prev" href="#category<?php
    echo $rowvc['id'];
?>" role="button" data-slide="prev" style="height: 90%;">
                <i class="fas fa-chevron-left"></i>
            </a>
            <a class="right carousel-control-next" href="#category<?php
    echo $rowvc['id'];
